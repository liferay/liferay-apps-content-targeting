...
<div class="content-container <%= containerCssClass %>">
			...
			<c:when test="<%= queryRule.hasAssetEntry() %>">
				<c:choose>
					...
					<c:otherwise>
						...
						<liferay-util:include page="/html/macros/render_asset_entry.jsp" servletContext="<%= application %>" />
					</c:otherwise>
				</c:choose>

				<liferay-util:include page="/html/common/analytics/track_content.jsp" servletContext="<%= analyticsServletContext %>">
					<liferay_util:param name="analyticsClassName" value="<%= queryRule.getAssetClassName() %>" />
					<liferay_util:param name="analyticsClassPK" value="<%= String.valueOf(queryRule.getAssetClassPK()) %>" />
	...
	<c:if test="<%= showPreview %>">
		...
		<liferay-util:include page="/html/macros/render_thumbnails_preview.jsp" servletContext="<%= application %>">
...
<liferay-util:include page="/html/macros/edit_icon_links.jsp" servletContext="<%= application %>">
	<liferay-util:param name="selectedIndex" value="<%= String.valueOf(selectedIndex) %>" />
</liferay-util:include>