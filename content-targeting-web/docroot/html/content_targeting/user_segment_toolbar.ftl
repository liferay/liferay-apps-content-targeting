...
<@aui["nav"]>
	<#if contentTargetingPermission.contains(permissionChecker, scopeGroupId, actionKeys.ADD_USER_SEGMENT)>
		...
		<@aui["nav-item"] href="${addUserSegmentURL}" iconCssClass="icon-plus" label='${languageUtil.get(portletConfig.getResourceBundle(locale), "add-user-segment")}' />
	</#if>

	<#if userSegmentPermission.contains(permissionChecker, scopeGroupId, scopeGroupId, actionKeys.DELETE)>
		<@aui["nav-item"] cssClass="hide" iconCssClass="icon-remove" id="deleteUserSegments" label='${languageUtil.get(portletConfig.getResourceBundle(locale), "delete")}' />
	</#if>
</@>