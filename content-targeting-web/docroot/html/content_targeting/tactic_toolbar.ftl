...
<@aui["nav"]>
	...
	<#if campaignPermission.contains(permissionChecker, campaign, actionKeys.UPDATE)>
		...
		<@aui["nav-item"] href="${addTacticURL}" iconCssClass="icon-plus" label='${languageUtil.get(portletConfig.getResourceBundle(locale), "add-promotion")}' />

		<@aui["nav-item"] cssClass="hide" iconCssClass="icon-remove" id="deleteTactics" label='${languageUtil.get(portletConfig.getResourceBundle(locale), "delete")}' />
	</#if>
</@>